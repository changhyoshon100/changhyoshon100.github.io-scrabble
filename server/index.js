...
app.post("/wordScore", (req, res) => {
  ...
  database.saveWordScore(word.name, word.word, word.score);
...
app.post("/highestWordScores", async (req, res) => {
  const dict = await database.top10WordScores();
  console.log(dict);
...
app.post("/gameScore", (req, res) => {
  ...
  const gameScore = database.saveGameScore(score.name, score.score);
  res.status(200).send(JSON.stringify(gameScore));
});
// TODO #7: Implement the /highestGameScores endpoint
app.post("/highestGameScores", (req, res) => {
  const highestGameScores = database.top10GameScores();
  res.status(200).send(JSON.stringify(highestGameScores));
});
